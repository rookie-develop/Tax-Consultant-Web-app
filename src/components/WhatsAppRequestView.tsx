  ...
  return (
    <div className="space-y-4 sm:space-y-6 max-w-3xl mx-auto w-full">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2.5 sm:gap-3 bg-white p-3 sm:p-4 rounded-xl border border-slate-200 shadow-2xs">
        ...
        <div className="flex items-center space-x-1.5 sm:space-x-2 text-[11px] sm:text-xs font-bold uppercase tracking-wider text-slate-400 min-w-0">
          <button
            id="breadcrumb-categories"
            type="button"
            onClick={onBackToCategories}
            className="hover:underline hover:text-slate-700 text-slate-500 py-1 cursor-pointer focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-emerald-500 rounded truncate max-w-[110px] min-[400px]:max-w-[150px] sm:max-w-none"
          >
            {category.title}
          </button>
          <span>/</span>
          <span className="text-slate-900 font-extrabold truncate max-w-[120px] min-[400px]:max-w-[180px] sm:max-w-xs">{subService.title}</span>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-xs overflow-hidden">
        {/* Header Block */}
        <div className="bg-slate-900 text-white p-4 sm:p-6 md:p-7 border-b border-slate-800">
        ...
        <div className="p-3.5 sm:p-6 md:p-8 space-y-5 sm:space-y-6">
          ...
          <div className="space-y-2 sm:space-y-2.5">
            <div className="flex items-center justify-between gap-2">
              ...
              <button
                ...
                className="inline-flex items-center space-x-1.5 text-xs font-bold px-2.5 sm:px-3 py-1.5 rounded-lg bg-slate-100 hover:bg-slate-200 active:bg-slate-300 text-slate-700 transition-colors cursor-pointer touch-manipulation focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500 min-h-[36px]"
              >
                {copied ? (
                  <>
            ...
            <div className="bg-[#EFEAE2] rounded-xl border border-[#DAD2C7] p-3 sm:p-4 md:p-5 shadow-inner">
              ...
              <div className="flex items-center justify-between pb-2.5 mb-2.5 sm:mb-3 border-b border-[#DFD7CD] text-xs text-slate-700 font-medium gap-2">
                <div className="flex items-center space-x-2 truncate min-w-0">
                  <div className="w-2.5 h-2.5 rounded-full bg-emerald-500 shrink-0" />
                  <span className="font-bold text-slate-800 truncate">To: {config.firmName}</span>
                  <span className="text-slate-500 font-mono text-[10px] sm:text-[11px] shrink-0 hidden min-[380px]:inline">({config.displayPhone})</span>
                </div>
                <span className="text-[10px] sm:text-[11px] text-slate-500 font-semibold uppercase tracking-wider shrink-0">
                  Preview
                </span>
              </div>
              ...
              <div className="flex justify-end">
                <div className="w-full sm:max-w-[95%] bg-[#DCF8C6] text-slate-900 rounded-2xl rounded-tr-xs p-3 sm:p-4 shadow-xs border border-[#C7E8B3] text-xs sm:text-sm font-sans whitespace-pre-wrap break-words leading-relaxed relative">
                  <div>{generatedMessage}</div>

                  {/* Timestamp & Blue Double Tick Indication */}
                  <div className="flex flex-wrap items-center justify-end gap-1.5 mt-2.5 pt-1.5 border-t border-[#CCE9B8] text-[10px] sm:text-[11px] text-slate-600 font-medium select-none">
          ...
          <AnimatePresence>
            {requestInitiated && (
              <motion.div
                ...
                className="p-3.5 sm:p-5 bg-emerald-50/95 border-2 border-emerald-500 rounded-xl flex items-center space-x-3 sm:space-x-3.5 shadow-2xs"
              >
                <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-emerald-500 text-slate-900 flex items-center justify-center shrink-0 shadow-xs">
                  <motion.div
                    initial={{ scale: 0 }}
                    animate={{ scale: 1 }}
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-xs sm:text-base font-black text-slate-900 leading-snug">
          ...
          <div className="pt-1 sm:pt-2">
            <a
              ...
              className="w-full bg-emerald-500 hover:bg-emerald-400 active:bg-emerald-600 text-slate-900 font-black px-4 sm:px-8 py-3.5 sm:py-4.5 rounded-xl flex items-center justify-center space-x-2 sm:space-x-3 uppercase tracking-wider text-xs min-[360px]:text-sm sm:text-base transition-all active:scale-[0.99] cursor-pointer shadow-sm hover:shadow-md min-h-[50px] sm:min-h-[58px] touch-manipulation focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500 focus-visible:ring-offset-2 text-center"
            >
              <MessageCircle className="w-4 h-4 sm:w-5 sm:h-5 fill-slate-900 shrink-0" />
